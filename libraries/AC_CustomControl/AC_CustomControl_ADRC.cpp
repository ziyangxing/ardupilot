...
Vector3f AC_CustomControl_ADRC::update(void)
{
    ...
    // const uint16_t radio_in = rc6->get_radio_in();//1000-2000 0.01-10   (0.01+9.99/1000 )*(x-1000) 
    // float tuning_value = radio_in/10.0f;
    // _rate_roll_cont.WC(tuning_value);    
    // _rate_pitch_cont.WC(tuning_value);
    // _rate_yaw_cont.WC(tuning_value);
    ...
    // gcs().send_text(MAV_SEVERITY_NOTICE, " roll wc:%f",_rate_yaw_cont.get_wc());
    return motor_out;
}
