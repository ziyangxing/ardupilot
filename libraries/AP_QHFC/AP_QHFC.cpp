...
{   
  ...
  if (_singleton != nullptr) {
      AP_HAL::panic("AP_QHFC must be singleton");
  }
#endif
  ...
  switch(buf[1])
  {
    case 0x03:
      if(len == 103)
      {
        ...
        HPSStatusV2_To_GC();
        gcs().send_text(MAV_SEVERITY_DEBUG, "HPSStatusV2");
      }
      else if(len == 47)
      {
        ...
        HPSStatusV1_To_GC();
        gcs().send_text(MAV_SEVERITY_DEBUG, "Humidity:%d,_FCV:%d,_FCA:%d,_FCW:%d,_FCWENDU1:%d,_FCKW:%d,_FCDCV:%d,_FCDCA:%d",HPSStatusV1.Humidity,HPSStatusV1._FCV,HPSStatusV1._FCA,HPSStatusV1._FCW ,HPSStatusV1._FCWENDU1,HPSStatusV1._FCKW,HPSStatusV1._FCDCV,HPSStatusV1._FCDCA);
      }
